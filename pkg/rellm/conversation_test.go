...
func TestFilesystemConversation_AppendAndLoadUnknownElement(t *testing.T) {
	...
	err := s.Append(ctx, []ConversationElement{newUnknownElement(string(ProviderTagOpenRouter), "future_output", "", raw)})
	assert.NoError(t, err)
	...
	assert.True(t, ok)
	assert.Equal(t, string(ProviderTagOpenRouter), unknown.Provider)
